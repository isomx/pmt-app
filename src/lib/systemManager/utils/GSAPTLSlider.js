...
//
export const GSAPTLSlider = function (tl, id, cnf) {
  this.config = {
    style: {
...
var pt = {
  container: function(el) {
    ...
    if (this.config.width) {
      ...
      this.railW = parseInt(this.config.rail.width, 10);
...
for (var k in pt) {
  if (pt.hasOwnProperty(k)) GSAPTLSlider.prototype[k] = pt[k];
}
// UTILS
function merge(target) {
  for (var i = 1; i < arguments.length; i++) {
    for (var k in arguments[i]) {
      if (arguments[i].hasOwnProperty(k)) target[k] = arguments[i][k];
    }
  }
};
var proxied = {};
...
function createEl(tag, att) {
  ...
  for (k in att) {
    if (!att.hasOwnProperty(k)) continue;
