    ...
    if (toResp && fromResp) {
      ...
      if (toCommon) {
        ...
        if (toResp && fromResp) {
          ...
          newBodyElems.push(
            { // copy of toCommon children (the common element, without rendering the MdTransitionElement wrapper)
              style: {
                position: 'absolute',
              ...
              children: toCommon.data.children,
              component: 'div',
              componentRef: (ref) => {
                commonCopyRef = ref;
              }
      ...
      let toAnchorRef, fromAnchorRef, toAnchorContainerRef, fromAnchorContainerRef, toCommonRef, fromCommonRef;
      if (reverse) {
        fromAnchorRender$ = fromAnchor.data.prepareRender(
          {
            position: 'absolute',
            top: `${0}px`,
            left: `${0}px`,
        ...
        animation$ = Rx.Observable.create((observer) => {
          ...
          let tl = new TimelineMax({paused: true, onComplete: completeCallback});
          toAnchorRef = toAnchor.data.getRef();
          fromAnchorRef = fromAnchor.data.getRef();
          fromAnchorContainerRef = fromAnchor.data.getContainerRef();
          toCommonRef = toCommon.data.getRef();
          if (!fromAnchorRef || !fromAnchorContainerRef) {
            observer.complete();
            return;
          }
          tl.set(unhideGroupChildRefs, {opacity: 1})
            .set(toCommonRef, {opacity: 0})
            ...
            .to(fromAnchorContainerRef, duration * .75, {
            ...
            .to(fromAnchorContainerRef, duration * .85, {
            ...
            // fromAnchorRef
            .to(fromAnchorRef, duration * .2, {opacity: 0}, 0)
        ...
        toAnchorRender$ = toAnchor.data.prepareRender(
          {
            position: 'absolute',
            top: `${0}px`,
            left: `${0}px`,
            ...
            zIndex: 2,
            visibility: 'visible',
            transform: `translate(${-toStyle.margin.left - toStyle.margin.right - toStyle.padding.left - toStyle.padding.right}px, ${-toStyle.margin.top - toStyle.margin.bottom - toStyle.padding.top - toStyle.padding.bottom }px`,
            opacity: 0,
          },
          {
            overflow: 'hidden',
        ...
        animation$ = Rx.Observable.create((observer) => {
          const completeCallback = () => observer.complete();
          const tl = new TimelineMax({paused: true, onComplete: completeCallback});
          toAnchorRef = toAnchor.data.getRef();
          toAnchorContainerRef = toAnchor.data.getContainerRef();
          fromAnchorRef = fromAnchor.data.getRef();
          toCommonRef = toCommon.data.getRef();
          fromCommonRef = fromCommon.data.getRef();
          if (!toAnchorRef || !toAnchorContainerRef) {
            observer.complete();
            return;
          }
          tl.set(unhideGroupChildRefs, {opacity: 1})
            .set(toCommonRef, {opacity: 0})
            .set(fromCommonRef, {opacity: 0})
            ...
            .to(toAnchorContainerRef, duration * .75, {
              width: toAnchor.data.rect.width,
              x: toAnchor.data.rect.left,
              ease: mdStandardEase,
            }, 0)
            .to(toAnchorContainerRef, duration * .85, {
              height: toAnchor.data.rect.height,
              y: toAnchor.data.rect.top,
              ease: mdStandardEase,
            }, duration * .15)
            .to(toAnchorContainerRef, duration * .2, {opacity: 1, onComplete: () => {tl.kill(null, fromAnchorRef)}}, 0)

            // toAnchorRef
            .to(toAnchorRef, duration * .333, {opacity: 1}, duration * 0.2)
            .to(toAnchorRef, duration * .75, {
              x: 0,
              ease: mdStandardEase,
            }, 0)
            .to(toAnchorRef, duration * .85, {
              y: 0,
              ease: mdStandardEase,
            }, duration * .15)
      ...
      const motionStream$ = Rx.Observable.concat(renderStream$, animation$, completeStream$);
